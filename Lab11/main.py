...
def Encode():
    test=np.frombuffer(b'Ala miala kota', dtype=np.uint8)
...
def Decode():
    in_file = open("./M0_11.hex", "rb") # opening for [r]eading as [b]inary
    data = in_file.read() # if you only wanted to read 512 bytes, do .read(512)
    in_file.close()
    # CorrectionCodes.Write(data,bit_count=11)
    DeCoded=CorrectionCodes.Read()

    print("Decoded:\n")
    for i in DeCoded.values():
        print(list(map(chr,i)))
